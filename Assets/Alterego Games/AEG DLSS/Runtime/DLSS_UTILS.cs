...
namespace AEG.DLSS
{
    ...
    {
        public DLSSQuality DLSSQuality = DLSSQuality.Balanced;
        public float m_antiGhosting = 0.1f;
#if AEG_DLSS && UNITY_STANDALONE_WIN && UNITY_64

        protected DLSSQuality m_previousDLSSQuality;
...
#region Public API
        ...
        /// </summary>
        public void OnSetQuality(DLSSQuality value) {
        ...
        public static void GetRenderResolutionFromQualityMode(out int renderWidth, out int renderHeight, int displayWidth, int displayHeight, NVIDIA.DLSSQuality qualityMode) {
        ...
        public static float GetUpscaleRatioFromQualityMode(NVIDIA.DLSSQuality qualityMode) {
            switch(qualityMode) {
        ...
        public static void UpdateDlssSettings(ref DlssViewData dlssSettings, ViewState state, NVIDIA.DLSSQuality _dlssQuality, UnityEngine.NVIDIA.GraphicsDevice _device) {

            dlssSettings.jitterX = -taaJitter.x;
            dlssSettings.jitterY = -taaJitter.y;
            dlssSettings.perfQuality = _dlssQuality;
